...
i tried to use gobuster and even did alot of things in that search engine but nothing worked,
until i saw the current version that is under the webpage. its says "Powered by Flask and 
Searchor 2.4.0" so now i look on google to see if theres a CVE for that version of Searchor.
...
and now we got a the shell, as always i upgrade the shell with 
```` python3 -c 'import pty; pty.spawn("/bin/bash")' ```` Ctrl + Z ```` stty raw -echo; fg ````
```` export TERM=xterm ````. Just like that we can have the user flag just doing 
...
````
We cant run it without arguments bcs of the * so i tryed run it with * and it showed this:
...
json format) 
eval() executes arbitrary Python code, so injecting code in the search query leads to RCE".
And i got this:
```
{                                                         
